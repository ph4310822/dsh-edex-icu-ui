...
3. **Refresh the rebuilt bundle into the clone** — `pnpm install` re-copies
   ...
   rm -rf node_modules/@danielng23/dsh-edex-icu-ui \
          node_modules/@danielng23/dsh-icu-client-ui-edex \
          node_modules/@danielng23/dsh-icu-client-ui-theme-terminal \
          node_modules/@danielng23/dsh-icu-host-system-metrics
   pnpm install
   ```
...
6. **Remove the plugin from the main profile** (the live 3080 server is
   ...
   ```sh
   cd /path/to/deepseek-harness
   pnpm dsh plugin --profile web remove @danielng23/dsh-edex-icu-ui
   ```
...
| 3081 | `web` | **npm production** — `@danielng23/dsh-edex-icu-ui@0.1.0` | verify the published package |
| 3083 | `web-edex` | **local** — `file:/path/to/dsh-edex-icu-ui/packages/bundle` | iterate on the checkout |
...
- **3083** is the local install from this checkout. The bundle's `file:`
  ...
  ```sh
  cd /path/to/deepseek-harness
  pnpm dsh plugin --profile web-edex add file:/path/to/dsh-edex-icu-ui/packages/bundle
...
curl -s -o /dev/null -w '%{http_code}\n' \
  http://127.0.0.1:3081/plugins/@danielng23/dsh-icu-client-ui-edex/client.js
```
