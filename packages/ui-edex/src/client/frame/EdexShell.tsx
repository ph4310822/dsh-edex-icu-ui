...
import type { CommandResult } from '@danielng23/dsh-icu-host-system-metrics/types'
...
/**
 * The workspace is inset below the center title strip by exactly the chrome
 * height (1px container border + 18px strip, whose own border sits inside the
 * strip height) so the strip frames the workspace without covering it
 * (AIRTRACK lesson).
 */
export const CENTER_TITLE_INSET = '19px'
 ...
 * vocabulary. The section itself stays transparent (XRY lesson — the card
 * chrome must never paint over the workspace); the 1px border + title strip
 * live on the `.centerWidget` container. `bleed` drops the chrome padding.
 */
...
export function EdexShell({
  ...
  useLayoutEffect(() => {
    ...
    // workspace is inset below the center title strip (CENTER_TITLE_INSET)
    // so the strip frames it without covering it.
    frame.style.top = `calc(${TOP_BAR_HEIGHT} + ${CENTER_TITLE_INSET})`
    frame.style.left = `calc(${CENTER_LEFT} + 1px)`
    frame.style.right = `calc(${CENTER_RIGHT} + 1px)`
    frame.style.bottom = `calc(${CENTER_BOTTOM} + 1px)`
  ...
  return (
    ...
    >
      ...
      {/* The ORIGINAL UI framed by the same widget chrome as the shell bars:
          a 1px border + the DSH WORKSPACE title strip on the container; the
          inner WidgetSection stays transparent so nothing covers the
          workspace (XRY lesson). */}
